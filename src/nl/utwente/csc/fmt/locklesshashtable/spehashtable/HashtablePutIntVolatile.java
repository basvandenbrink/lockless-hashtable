...
public class HashtablePutIntVolatile {
	...
	private static final void storeHash(int[] hashes, int i, int hash){
		unsafe.putIntVolatile(hashes, rawIndex(hashes, i),
				hash);
	}
	 ...
	 */
	public HashtablePutIntVolatile() {
		this(MIN_SIZE);
	}
	 ...
	 */
	public HashtablePutIntVolatile(final int initial_sz) {
	...
	private final void initialize(int initial_sz) {
		...
		for (i = MIN_SIZE_LOG; (1 << i) < (initial_sz << 1); i++)
			;
		hvs = new int[1 << i][];
		hashes = new int[1 << i]; // Matching hash entries
	}
	...
	private static boolean valEq(int[] vector1, int[] vector2, int[] hashes,
	...
	public boolean lookup(int[] vector) {
		...
		while (true) {
			...
			// CAS failed
			v = val(hvs, idx);
			while(hashes[idx] == 0); //spinlock until hash value is set
